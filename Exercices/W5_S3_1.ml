...
let output_multiples x n m =
  for i=n to m do
    if(is_multiple i x) then (print_int i;print_char ',')
    else ()
  done;;
...
let display_sign_until_zero f m =
  try
    for i=0 to m do
      let res = f i in
      let machin r =
        if (r > 0) then (print_string "positive";print_newline())
        else if (r < 0) then (print_string "negative";print_newline())
        else raise Toto
      in
      machin res
    done
  with toto -> print_string "zero";;
