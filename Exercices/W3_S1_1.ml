...
*)
let is_empty (front, back) = List.length front = 0 && List.length back = 0;;
...
let split l =
  ...
  let rec split' i l' ls=
    match l' with
    | [] -> ([],[])
    | (x::xs) ->
        if i < mid
        then split' (i+1) xs (ls @ [x])
...
let dequeue (front, back) =
  ...
  | ((x::xs),_) -> (x,(xs,back))
  | _ -> let (x::xs) =List.rev back in (x,(front,List.rev xs));;
