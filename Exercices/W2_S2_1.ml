(*
POINTS AND VECTORS  (28/30 points)
...
let move (p:point) (dp:dpoint) = {x = p.x +. dp.dx; y= p.y +. dp.dy; z= p.z +. dp.dz};;

let next po = {position = move po.position po.velocity; velocity = po.velocity};;
...
let will_collide_soon po1 po2 =
  dist (next po1).position (next po2).position < 1.0;;
