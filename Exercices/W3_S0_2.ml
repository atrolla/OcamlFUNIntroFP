...
*)
let read a index =
  match index with | Index i -> a.(i);;

let inside a index =
  match index with | Index i -> i >= 0 && i < Array.length a;;

let next (index:index) =
  match index with | Index i -> Index (i+1);;

let min_index a =
  let rec min' a mindex index =
    if not (inside a index)
    then mindex
    else
    if (read a mindex) > (read a index)
    then min' a index (next index)
    else min' a mindex (next index)
  in
  min' a (Index 0) (Index 1);;
