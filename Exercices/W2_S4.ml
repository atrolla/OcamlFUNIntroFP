...
let proof_of_bug =
  ...
  [| {code = 0;contact = { name = "JohnDoe"; phone_number = (1, 1, 1, 1) }};
     {code = 0;contact = { name = "JohnDoe2"; phone_number = (2,2,2,2) }};
     {code = 0;contact = { name = "JohnDoe3"; phone_number = (3,3,3,3) }};
     {code = 1;contact = { name = "JohnDoe2"; phone_number = (2, 2, 2, 2) }};
     {code = 2;contact = { name = "JohnDoe3"; phone_number = (3,3,3,3) }}
  |] ;;
...
let delete db contact =
  ...
  if not status then (false, db, contact)
    ...
    let db' =
      let rec copy thedb i =
        if i = Array.length db.contacts
        then thedb
        else if (db.contacts.(i).name <> contact.name && db.contacts.(i) <> nobody) then
          let (_,newdb,_) = insert thedb db.contacts.(i) in
          copy newdb (i+1)
        else copy thedb (i+1)
      in
      copy (make (Array.length db.contacts)) 0
    in
    (true, db', contact);;
...
let update db contact =
  let (status, db, _) = search db contact in
  if status
  then let (_,newdb,_) = delete db contact in
    insert newdb contact
  else
    insert db contact;;
