...
let compare user reference to_string:message = match user,reference with
    (Ok a, Ok b) -> if (a=b) then ("got correct value "^(to_string a), Successful) else ("got unexpected value "^(to_string a), Failed)
  |  (Error a, Error b) ->if (a=b) then ("got correct exception "^(exn_to_string a), Successful) else ("got unexpected exception "^(exn_to_string a), Failed)
...
let test user reference sample to_string:report =
  let rec foo x acc = if x = 0
    then List.rev acc
    else let samp = sample () in
      let nacc = compare (exec user samp) (exec reference samp) to_string :: acc in
      foo (x-1) nacc
  in
  let i = 10 in
  foo i [];;
